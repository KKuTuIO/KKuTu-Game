...
process.on('message', function (msg) {
    switch (msg.type) {
        case "invite-error":
            ...
            break;
        case "room-reserve":
            IOLog.info(`메인 채널로부터 ${msg.room.id}번 방의 예약 요청이 접수되었습니다.`);
            if (RESERVED[msg.session]) {
                delete RESERVED[msg.session];
                IOLog.warn(`${msg.room.id}번 방의 예약 요청이 이미 존재합니다. 진행 중인 입장 요청을 삭제합니다.`);
                break;
                ...
                _expiration: setTimeout(function (tg, create) {
                    ...
                    IOLog.info(`방 생성 요청(${msg.room.id})이 만료되었습니다.`);
                    delete RESERVED[tg];
