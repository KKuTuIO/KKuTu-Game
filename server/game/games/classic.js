...
export function getTitle () {
    ...
    switch (GAME_TYPE[my.mode]) {
        ...
        case 'KFT':
            if (GAME_TYPE[my.mode] === 'KFT') my.game.wordLength = 4;
        case 'KSH':
        case 'KAP':
            my.game.minLength = my.opts.no2 ? 3 : 2;
...
export function turnEnd () {
    ...
    if(my.game.attackInfo && my.game.attackInfo.remainingWordCount){
        if (my.game.attackInfo.remainingWordCount <= 12) stack = true;
        const getBonusCoeff = dist => (52 - 4 * my.game.attackInfo.remainingWordCount) * 0.5 ** dist;
...
export function submit (client, text) {
    ...
    function isChainable() {
        ...
        if (my.game.minLength && text.length < my.game.minLength) return false;
...
export function useItem (client, id) {
    ...
    if ((my.game.turnTime - ((new Date()).getTime() - my.game.turnAt)) < my.game.turnTime / 5)
        return client.publish('turnError', { code: 422 }, true); // 제한시간 초과
...
export function getScore (text, delay, ignoreMission) {
    ...
    if (!ignoreMission) {
        if (arr = text.match(new RegExp(my.game.mission, "g"))) {
            const bonus = getMissionBonusMultiplier(arr.length, my.opts.tactical);
            score += score * bonus;
...
// 택티컬 모드 보너스 계산
function getTacticalBonus(x) {
    switch (true) {
        case x >= 1 && x <= 3:
            return 1.20 - 0.20 * x; // x=1 → 1.00, x=2 → 0.80, x=3 → 0.60
        case x >= 4 && x <= 8:
            return 0.60 - 0.10 * (x - 3); // x=4 → 0.50, x=5 → 0.40, x=6 → 0.30, x=7 → 0.20, x=8 → 0.10
        case x === 9:
            return 0.05;
        default:
            return 0.01;
...
// 일반 모드 보너스 계산
function getNormalBonus(x) {
    switch (true) {
        case x >= 1 && x <= 3:
            return 0.60 - 0.10 * x; // x=1 → 0.50, x=2 → 0.40, x=3 → 0.30
        case x === 4:
            return 0.25;
        case x >= 5 && x <= 8:
            return 0.25 - 0.05 * (x - 4); // x=5 → 0.20, x=6 → 0.15, x=7 → 0.10, x=8 → 0.05
        default:
            return 0.01;
    }
}

// 글자 수(count)와 모드에 따라 누적 보너스를 계산 (배열 메서드 사용, 소숫점 2자리 반올림)
function getMissionBonusMultiplier(count, isTactical) {
    const bonus = Array.from({ length: count }, (_, i) => i + 1)
        .reduce((sum, x) => sum + (isTactical ? getTacticalBonus(x) : getNormalBonus(x)), 0);
