...
DB.onReady(function () {
    ...
    for (i in MAPS) {
        MC[MAPS[i].name] = 0;
        MAPS[i].queue = MAPS[i].queue.split(' ').map(function (item) {
            return item.split('');
        });
    }
    ...
    function doMining() {
        getBoard(LANG).then(function (data) {
            ...
            Prompt.get(['flush'], function (err, _res) {
                if (_res.flush == "y") {
                    MC[data.map.name]++;
                    DB.kkutu_cw[LANG].insert(['map', data.map.name], ['data', res.map(function (item) {
                        return item.join(',');
                    }).join('|')]).on();
                }
...
let words = ["성교", "음경", "지랄", "불알", "자위", "자지", "보지", "보장지", "개새끼", "성관계", "고자", "창녀"];
...
function getBoard(lang) {
    let R = new Tail();
    let MEAN = ['mean', new RegExp("^.{9}[^=→][^\.]{15}")];
    let NO_BUL = new RegExp("^(500|210|120|10)$");
    let board = {};
    let proc = [];
    let map = getMap();
    ...
    function process() {
        ...
        if (regCache[reg]) {
            ...
            // regCache[reg] = $docs;
            regCache[reg] = $docs.sort(function (a, b) {
                return b.hit - a.hit;
            });
            // if(Math.random() < 0.5 && regCache[reg].length >= 50) regCache[reg].shift();
            onDBFound(regCache[reg].shift());
