...
import { init as RPInit } from '../sub/utils/ProcessReport.js';
...
export function getWeightedRandom (arr) {
    let total = Object.values(arr).reduce((a, b) => a + b, 0);
...
export class Client {
    constructor (socket, profile, sid) {
        ...
        socket.on('message', (msg) => {
            ...
            if (!data.hasOwnProperty('type') || (data.type !== 'wms' && data.type !== 'drawingCanvas')) {
    ...
    */
    drawingCanvas (msg) {
        ...
        $room.drawingCanvas(msg, this.id);
    };
    ...
    canvasNotValid (msg) {
        ...
        this.send('drawCanvas', {
            diffed: false,
            image: true,
            data: $room.fullImageString
        });
    }
    getData (gaming) {
        let o = {
    ...
    drawingCanvas (msg, userId) {
        if(this.game.painter == userId) { // verify this data sended by painter
            /*
            let diffed = true;
            let diff, diffResult;
            ...
            if (msg.diffed) {
                ...
                diffResult = differ.patch_apply(diff, this.game.fullImageString);

                if(diffResult[1]) {
                    this.game.fullImageString = diffResult[0];
                } else {
                    this.byMaster('diffNotValid', {}, true);
                }
            } else {
                diffResult = msg.data;
                diffed = false;
            ...
            this.byMaster('drawCanvas', { diffed, data: msg.data }, true);
            */
            if (
                msg.data && msg.data.startsWith('data:image/jpeg;base64,') &&
                isValid(msg.data.substring('data:image/jpeg;base64,'.length)) &&
                msg.data.length <= 160000
            ) {
                this.byMaster('drawCanvas', {
                    diffed: false,
                    image: true,
                    data: msg.data
                });
                this.game.fullImageString = msg.data;
            }
        }
    };
    ...
    flush (box, equip, friends, flags, createdat, retryCount) {
        let R = new Tail();
        ...
        DB.users.findOne(['_id', this.id]).on((currentUser) => {
            ...
            DB.users.upsert(['_id', this.id]).set(
                ...
                createdat ? ['createdat', this.createdat] : undefined
            ).on((__res) => {
                ...
                if (curr.from == "fix") addResult(curr.type, curr.value * r)
                    ...
                    if (curr.from == "per") {
                        ...
                        while (r-- > 0) {
                            ...
                            addResult(curr.type, Math.round(
                                Math.min(Math.sqrt(4.8 * base) / 2, 64) +
                                (Math.sqrt(80 + 3.0 * base) / 2)
                            ));
                        }
                    }
                }
    ...
    enter (room, spec, pass) {
        ...
        if (this.place) {
            ...
            if (!this.admin) {
                if ($room.opts.onlybeginner && (getLevel(this) >= 50 || this.guest)) {
                ...
                if ($room.opts.noobserver && $room.gaming) return this.sendError(705);
                if ($room.players.length >= $room.limit + ($room.gaming ? MAX_OBSERVER : 0)) {
            ...
            if (!this.admin && $room.players.length >= $room.limit + (spec && $room.gaming ? MAX_OBSERVER : 0)) {
                return this.sendError(429);
            }
    ...
    byMaster (type, data, nob) {
        const $room = ROOM[this.place];
        if ($room.master && DIC[$room.master]) DIC[$room.master].publish(type, data, nob);
    };
};
...
export class Room {
            ...
            if (this.gaming) {
                ...
                if (x != -1) {
                    ...
                    if (this.game.turnAt && this.game.turnTime) {
                        const elapsed = Data.now() - this.game.turnAt;
    ...
    roundEnd (data) {
        ...
        if (this.game.robots !== undefined && (this.game.seq.length - this.game.robots.length) < 2) rl = 1;
...
function getRewards(mode, score, bonus, rank, all, ss, membership) {
    ...
    let pos = (all - rank) > 8 ? 8 + ((all - rank - 8) * 0.3) : (all - rank);
